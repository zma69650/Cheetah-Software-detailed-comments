...
void MiniCheetahHardwareBridge::run() {
  //TODO step 1. 初始化通信模块，调用基类HardwareBridge::initCommon()函数，通过LCM通信模块订阅interface和interface_request两个主题
  initCommon();
  //TODO step 2. 初始化MiniCheetah机器人的spi通信和惯性导航模块(IMU)
  initHardware();
  //TODO step 3. 初始化机器人参数和用户控制参数列表
  ...
  //TODO step 4. 创建和配置RobotRunner对象（实际上是一个任务对象）
  ...
  // rc controller
  //TODO 启动遥控器指令接收任务 (_gamepadCommand)
